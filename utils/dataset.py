...
def collate_fn(
    ...
    for conversation, target in zip(conversation_list, targets):
        ...
        rounds = [conversation] if conv.sep2 is None else conversation.split(conv.sep2)
        #rounds = conversation.split(conv.sep)
        cur_len = 0 if conv.sep2 is None else 1
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.datasets = dataset.split("||")[-1:] ## Have dataset only for reason_seg
        if len(self.datasets) == 1:
            self.sample_rate = [1.0]
        else:
            self.sample_rate = np.array(self.sample_rate)
